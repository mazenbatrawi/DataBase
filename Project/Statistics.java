...
class sortComp implements Comparator<D> {
    ...
    public int compare(D a, D b){
        ...
        int day1 = (date1.charAt(0) - '0') * 10 + date1.charAt(1) - '0';
        int month1 = (date1.charAt(3) - '0') * 10 + date1.charAt(4) - '0';
        int year1 = (date1.charAt(6) - '0') * 1000 + (date1.charAt(7) - '0') * 100 + (date1.charAt(8) - '0') * 10 +
                (date1.charAt(9) - '0');

        int day2 = (date2.charAt(0) - '0') * 10 + date2.charAt(1) - '0';
        int month2 = (date2.charAt(3) - '0') * 10 + date2.charAt(4) - '0';
        int year2 = (date2.charAt(6) - '0') * 1000 + (date2.charAt(7) - '0') * 100 + (date2.charAt(8) - '0') * 10 +
                (date2.charAt(9) - '0');
...
public class Statistics implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        while(true){
            ...
            assert date != null;
            if(date.length() == 10 && date.charAt(2) == date.charAt(5) && date.charAt(2) == '-'){
                day = (date.charAt(0) - '0') * 10 + date.charAt(1) - '0';
                month = (date.charAt(3) - '0') * 10 + date.charAt(4) - '0';
                year = (date.charAt(6) - '0') * 1000 + (date.charAt(7) - '0') * 100 + (date.charAt(8) - '0') * 10 +
                        (date.charAt(9) - '0');
            }
            double cost = 0;
            try {
            ...
            D d = null;
            try {
                d = new D(resultSet.getString(1));
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
            if(hashMap.containsKey(d)){
                double temp = hashMap.get(d);
                temp += cost;
                hashMap.put(d, temp);
            }
            else{
        ...
        for(D g : arrayList){
            series.getData().add(new XYChart.Data<String, Double>(g.toString(), hashMap.get(g)));
        }
