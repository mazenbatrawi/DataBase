...
public class SearchBill {
    ...
    void search(ActionEvent event) throws SQLException, ClassNotFoundException {
        if(!bill_id_button.isSelected() && !employee_id_button.isSelected() && !date_button.isSelected() &&
        ...
        else if(sbill_id.getText().isEmpty() && semployee_id.getText().isEmpty() && sdate.getText().isEmpty() &&
        ...
        else if(!sdate.getText().isEmpty() && (sdate.getText().length() != 10 ||
                (sdate.getText().length() == 10 &&
                        (sdate.getText().charAt(2) != '-' || sdate.getText().charAt(5) != '-')))){
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText(null);
            alert.setContentText("Please Check The Date Format Of The Bill (dd-mm-yyyy)");
            alert.showAndWait();
        }
        else{
            query = "select ";
            ...
            if (!sdate.getText().isEmpty()) query += "datee = \"" + sdate.getText() + "\" and ";
